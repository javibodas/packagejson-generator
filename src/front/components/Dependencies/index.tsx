...
import Badge from 'src/front/components/Badge'
import FileCtx from 'src/front/state/file'
import useDependencies from 'src/front/hooks/useDependencies'
	...
	return (<div className='flex flex-col max-h-52'>
		<div className='grid grid-cols-5 py-1.5 px-0'>
			...
			<section className='section-input col-start-2 col-end-6'>
				<input id = {'inpt-dependencies' + type} className = 'w-full py-1 px-0 input-dependencie border-b-1 border-solid border-gray-200 hover:border-garnet hover:outline-none focus:border-garnet focus:outline-none rounded-none lg:text-base text-[10px]' placeholder="NPM Package" onChange={typePackage} data-testid={'input-' + classType}/>
				{ isPackagesOpen &&
					(<div id = {'packlist' + type} className='text-black text-xs bg-white max-h-[100px] fixed z-10 w-400 transition-all delay-75 overflow-y-scroll' data-testid={'combo-' + classType}>
